...
public:
    ...
    using FnCreateAdafruit_EPD = std::function<Adafruit_EPD *(int16_t, int16_t, int16_t, int16_t, int16_t, thinkinkmode_t)>;

    static inline const std::map<std::string, FnCreateAdafruit_EPD> adafruitEPDFactory = {
      // Monochrome
      {"154-mono-D27",
      ...
      {"29-gray-EAAMFGN",
       [](int16_t dc, int16_t rst, int16_t cs, int16_t sram_cs, int16_t busy,
         ...
         ThinkInk_290_Grayscale4_EAAMFGN *d =
             new ThinkInk_290_Grayscale4_EAAMFGN(dc, rst, cs, sram_cs, busy);
         d->begin(mode);
         return d;
       }},
      {"29-gray-FPC7519",
       [](int16_t dc, int16_t rst, int16_t cs, int16_t sram_cs, int16_t busy,
         ...
         ThinkInk_290_Grayscale4_FPC7519 *d =
             new ThinkInk_290_Grayscale4_FPC7519(dc, rst, cs, sram_cs, busy);
         d->begin(mode);
         return d;
       }},
      {"29-gray-T5",
  ...
  /*!
      ...
      @param  epdMode    The EPD mode (e.g., ws_display_EPDMode_EPD_MODE_GRAYSCALE4).
  ...
  */
  drvDispThinkInk(char *epdPanel, ws_display_EPDMode epdMode, int16_t dc, int16_t rst, int16_t cs, int16_t sram_cs = -1, int16_t busy = -1)
  ...
  */
  bool begin() override {
    ...
    // The factory constructs the concrete panel and runs its typed begin(mode)
    ...
    _display->clearBuffer();
    // TODO: This probably doesn't need to be here, but there isnt a base class for thinkink w/marquee yet
