/*!
 * @file src/components/display/drivers/dispDrvThinkInkMonoAAAMFGN.h
 *
 * Driver for ThinkInk 5.83" Monochrome AAAMFGN display (UC8179)
 *
 ...
 *
 */
#ifndef WS_DRV_THINKINK_MONO_AAAMFGN_H
#define WS_DRV_THINKINK_MONO_AAAMFGN_H

#include "dispDrvBase.h"

/*!
    @brief  Driver for a ThinkInk 5.83" Monochrome AAAMFGN display (UC8179).
*/
class dispDrvThinkInkMonoAAAMFGN : public dispDrvBase {
public:
  /*!
      @brief  Constructor for the ThinkInk 5.83" Monochrome AAAMFGN driver.
  ...
  */
  dispDrvThinkInkMonoAAAMFGN(int16_t dc, int16_t rst, int16_t cs,
                             int16_t sram_cs = -1, int16_t busy = -1)
      : dispDrvBase(dc, rst, cs, sram_cs, busy), _display(nullptr) {}

  ~dispDrvThinkInkMonoAAAMFGN() {
    if (_display) {
  ...
  /*!
      ...
      @param  reset  True to perform a hardware reset (unused here).
      @return True if initialization succeeded, False otherwise.
  */
  bool begin(thinkinkmode_t mode, bool reset = true) override {
    (void)reset;
    _display = new ThinkInk_583_Mono_AAAMFGN(_pin_dc, _pin_rst, _pin_cs,
  ...
  */
  ...
  /*!
      ...
      @param  bat          Current battery level (0-100, unused here).
...
private:
  ThinkInk_583_Mono_AAAMFGN *_display;
};

#endif // WS_DRV_THINKINK_MONO_AAAMFGN_H
