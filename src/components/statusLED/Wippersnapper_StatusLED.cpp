...
void initStatusLED() {
  ...
  statusPixel->show();
  WS.lockStatusNeoPixel = true; // lock pixel for use
#endif
...
*/
/****************************************************************************/
void fadeStatusLED(uint32_t color, int numFades = 3) {
...
*/
/****************************************************************************/
void blinkStatusLED(ws_led_status_t statusState) {
#ifdef USE_STATUS_LED
  if (!WS.lockStatusLED)
    return;
#endif
