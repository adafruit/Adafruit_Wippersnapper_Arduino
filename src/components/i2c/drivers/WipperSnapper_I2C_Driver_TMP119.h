  ...
  */
  ...
  bool begin() {
    ...
    if (!_tmp119->begin((uint8_t)_sensorAddress, _i2c))
      return false;
    // Set defaults explicitly — library reset() sets these, but we don't
...
protected:
  Adafruit_TMP119 *_tmp119 = nullptr; ///< Pointer to TMP119 sensor object

  sensors_event_t _cachedTemp = {0}; ///< Cached temperature reading
  ...
  /*!
      @brief    Reads sensor data, with 1-second cache to avoid redundant
                ...
                (e.g. degrees C then degrees F). The calling order of getEvent
                methods is not guaranteed, so every method must go through this
                function.
