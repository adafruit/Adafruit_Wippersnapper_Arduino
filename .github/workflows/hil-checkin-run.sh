...
{
  echo "<!-- hil-test-suite -->"
  echo "## 🔌 HIL check-in smoke test"
  echo
  echo "Flash this PR's build → write secrets → power-cycle → assert the device checks in to the broker. (The pixelWrite #926/#927 A/B regression is currently parked.)"
  echo
...
{
  echo
  echo "_Expected: \`ok=true\` (device flashed + configured + checked in). Proof (serial.log / protomq.log / flash.log) attached as the \`hil-assets\` artifact._"
  echo
  echo "<sub>Driven by usbip-hil-controller over Tailscale; only bench-available targets run (others skipped+listed). 🤖 Generated by the HIL test suite.</sub>"
} >> hil-out/comment.md
